...
async function runAllProcesses() {
    ...
    log('Serving the legacy nuxt application...');
    runNpmScript(path.join(appDir, 'legacy-app-nuxt-v2'), 'dev.nuxt');

    await sleep(2000);

    log(`Serving the gateway worker...`);
    runNpmScript(path.join(appDir, 'legacy-app-nuxt-v2'), 'dev.worker');
    await sleep(2000);
